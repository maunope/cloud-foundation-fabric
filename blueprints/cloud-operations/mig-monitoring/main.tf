...
module "pubsub" {
  ...
  project_id = local.monitoring_project
  name       = "mig-monitoring-pubsub"
  subscriptions = {
    "mig-monitoring-pubsub-default" = null
  }
...
module "cloud-function" {
  ...
  bucket_name = "${local.monitoring_project}-mig-monitoring-bucket"
  bucket_config = {
    location             = var.region
...
resource "google_monitoring_dashboard" "dashboard" {
  dashboard_json = file("${path.module}/dashboards/min-monitoring.json")
  project        = local.monitoring_project
}